...
echo "$BD" | grep -q 'hyprctl dispatch' && ok "boundary prints hyprctl recipe" || bad "boundary missing hyprctl"
BDRY="$("$ROOT/bin/bluechip-scratchpad" --boundary --dry-run)"
echo "$BDRY" | grep -q 'togglespecialworkspace bluechip-boundary' && ok "boundary dry-run names workspace" || bad "boundary dry-run workspace: $BDRY"
